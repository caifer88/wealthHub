...
export default function App() {
  ...
  const { darkMode, setDarkMode, assets, history, bitcoinTransactions, stockTransactions, downloadBackup } = useWealth()
          ...
          <aside className={`${sidebarOpen ? 'block' : 'hidden'} md:block w-full md:w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 p-4`}>
            <nav className="space-y-2">
              ...
              <div className="mt-8 pt-6 border-t border-slate-200 dark:border-slate-800">
                <button
                  onClick={() => downloadBackup(assets, history, bitcoinTransactions, stockTransactions)}
